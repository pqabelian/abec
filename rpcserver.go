...
var (
	...
	// data.
	gbtCoinbaseAux = &abejson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &abejson.RPCError{
	...
	ErrRPCNoWallet = &abejson.RPCError{
		Code:    abejson.ErrRPCNoWallet,
...
// should ask a connected instance of abewallet.
...
func rpcDecodeHexError(gotHex string) *abejson.RPCError {
	return abejson.NewRPCError(abejson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *abejson.RPCError {
	return abejson.NewRPCError(abejson.ErrRPCNoTxInfo,
...
func rpcUtxoRingNoInfoError(outPointRing *wire.OutPointRing) *abejson.RPCError {
	return abejson.NewRPCError(abejson.ErrRPCUtxoRingNoInfoError,
		fmt.Sprintf("No information available about utxoRing %v",
			outPointRing.String()))
}

func rpcUtxoRingMismatchedInfoError(outPointRing *wire.OutPointRing) *abejson.RPCError {
	return abejson.NewRPCError(abejson.ErrRPCUtxoRingMismatchedInfoError,
		fmt.Sprintf("UtxoRing is found but mismatched with the queried outpointring %v",
			outPointRing.String()))
}
...
type gbtWorkState struct {
	...
	template  *reqTemplate            // for configured addresses
	templates map[string]*reqTemplate // for request with specified address
...
func newGbtWorkState(timeSource blockchain.MedianTimeSource) *gbtWorkState {
	return &gbtWorkState{
		...
		templates:  make(map[string]*reqTemplate),
...
// These commands will be implemented in abewallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		//	todo(ABE): This is a bug, since here the amount is in BTC.
		if amount <= 0 || amount > abeutil.MaxSatoshi {
			return nil, &abejson.RPCError{
		...
		addr, err := abeutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &abejson.RPCError{
		...
		satoshi, err := abeutil.NewAmount(amount)
		if err != nil {
...
func handleCreateRawTransactionAbe(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	mtx := wire.NewMsgTxAbe(wire.TxVersion)
	...
	for _, input := range c.Inputs {
		...
		for i := 0; i < blockHashsNum; i++ {
			blockHash, err := chainhash.NewHashFromStr(input.PreviousOutPointRing.BlockHashs[i])
			if err != nil {
				return nil, rpcDecodeHexError(input.PreviousOutPointRing.BlockHashs[i])
			}
			blockHashs[i] = blockHash
		}
		...
		for i := 0; i < ringSize; i++ {
			txHash, err := chainhash.NewHashFromStr(input.PreviousOutPointRing.OutPoints[i].Txid)
			if err != nil {
				return nil, rpcDecodeHexError(input.PreviousOutPointRing.OutPoints[i].Txid)
			}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*abejson.TxRawResult, error) {
	...
	txReply := &abejson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(abeutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(abeutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txReply := abejson.TxRawDecodeResult{
...
func handleDecodeRawTransactionAbe(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Create and return the result.
	txReply := abejson.TxRawDecodeResultAbe{
		Txid:    mtx.TxHash().String(),
		Version: mtx.Version,
		Vin:     createVinListAbe(&mtx),
		Vout:    createVoutListAbe(&mtx, s.cfg.ChainParams),
		Fee:     abeutil.Amount(mtx.TxFee).ToABE(),
	}
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := abeutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := abejson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abejson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &abejson.RPCError{
				Code:    abejson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*abejson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result abejson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = abejson.Bool(peer.Connected())
		...
		addrs := make([]abejson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr abejson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abejson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &abejson.RPCError{
			Code:    abejson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abejson.GetBlockHeaderCmd)
...
func (state *gbtWorkState) NotifyBlockConnected(blockHash *chainhash.Hash) {
	go func() {
		...
		if state.template != nil {
			state.notifyLongPollers(blockHash, state.template.lastTxUpdate)
		}
		for _, template := range state.templates {
			state.notifyLongPollers(blockHash, template.lastTxUpdate)
		}
	}()
}
...
func (state *gbtWorkState) NotifyMempoolTx(lastUpdated time.Time) {
	go func() {
		...
		// yet.
		if state.template == nil ||
			state.template.prevHash == nil || state.template.lastGenerated.IsZero() {
			return
		}

		if time.Now().After(state.template.lastGenerated.Add(time.Second *
			gbtRegenerateSeconds)) {

			state.notifyLongPollers(state.template.prevHash, lastUpdated)
		}

		for _, template := range state.templates {
			// No need to notify anything if no block templates have been generated
			// yet.
			if template == nil || template.prevHash == nil || template.lastGenerated.IsZero() {
				return
			}

			if time.Now().After(template.lastGenerated.Add(time.Second *
				gbtRegenerateSeconds)) {

				state.notifyLongPollers(template.prevHash, lastUpdated)
			}
		}
...
func GetExtraNonceOffset(tx *wire.MsgTxAbe) (int64, error) {
	...
	snLen, err := abecryptoxparam.GetSerialNumberSerializeSize(tx.TxIns[0].PreviousOutPointRing.Version)
	if err != nil {
		return 0, err
	}
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, consensusApplied wire.ConsensusProtocol, useOwnAddr bool, ownMiningAddr abeutil.AbelAddress) error {
	...
	if template == nil ||
		...
		(template.lastTxUpdate != lastTxUpdate &&
			time.Now().After(template.lastGenerated.Add(time.Second*gbtRegenerateSeconds))) {
		...
		template.template = blkTemplate
		template.lastGenerated = time.Now()
		template.lastTxUpdate = lastTxUpdate
...
func (state *gbtWorkState) blockTemplateResult(miningAddr abeutil.AbelAddress, submitOld *bool) (*abejson.GetBlockTemplateResult, error) {
	...
	if template == nil {
		return nil, &abejson.RPCError{
			Code: abejson.ErrRPCInternal.Code,
			Message: "A block template has been requested, " +
				"but the server is not prepared to respond appropriately.",
		}
	}
...
func handleGetBlockTemplateLongPoll(s *rpcServer, longPollID string, useCoinbaseValue bool, closeChan <-chan struct{}) (interface{}, error) {
	...
	prevTemplateHash := &state.template.template.Block.Header.PrevBlock
	if !prevHash.IsEqual(prevTemplateHash) ||
		lastGenerated != state.template.lastGenerated.Unix() {
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := abejson.NewGetNetworkHashPSCmd(nil, nil)
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &abejson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abejson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*abejson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &abejson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abejson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	rawTxn, err := createTxRawResultAbe(s.cfg.ChainParams, mtx, txHash.String(),
...
/*func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &abejson.RPCError{
				Code: abejson.ErrRPCInvalidTxVout,
...
func handleGetUtxoRing(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blockHashs := make([]*chainhash.Hash, blockHashsNum)
	for i := 0; i < blockHashsNum; i++ {
		blockHash, err := chainhash.NewHashFromStr(c.OutPointRing.BlockHashs[i])
		if err != nil {
			return nil, rpcDecodeHexError(c.OutPointRing.BlockHashs[i])
		}
		blockHashs[i] = blockHash
	}
	...
	outPoints := make([]*wire.OutPointAbe, ringSize)
	for i := 0; i < ringSize; i++ {
		txHash, err := chainhash.NewHashFromStr(c.OutPointRing.OutPoints[i].Txid)
		if err != nil {
			return nil, rpcDecodeHexError(c.OutPointRing.OutPoints[i].Txid)
		}
	...
	if err != nil {
		return nil, rpcUtxoRingNoInfoError(outPointRing)
	}

	// To match the behavior of the reference client, return nil
	// (JSON null) if the the txoring is 'all' spent in the main chain.
	if utxoRingEntry == nil || utxoRingEntry.IsAllSpent() {
		return nil, nil
	}

	ringHashInCmd := outPointRing.Hash()
	ringHashFetched := utxoRingEntry.OutPointRing().Hash()
	if !ringHashFetched.IsEqual(&ringHashInCmd) {
		return nil, rpcUtxoRingMismatchedInfoError(outPointRing)
	}

	utxoRingReply := &abejson.GetUtxoRingResult{
		RingBlockHeight: utxoRingEntry.RingBlockHeight(),
		OutPointRing:    c.OutPointRing, //	directly use the OutPointRing in the cmd
		IsCoinbase:      utxoRingEntry.IsCoinBase(),
	}

	if len(utxoRingEntry.TxOuts()) != ringSize {
		return nil, rpcUtxoRingMismatchedInfoError(outPointRing)
	}
	...
	consumedSnNum := len(utxoRingEntry.SerialNumbers())
	...
	utxoRingReply.ConsumingBlockHashs = consumingBlockHashs

	return utxoRingReply, nil
}

// handleHelp implements the help command.
func handleHelp(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abejson.HelpCmd)
...
func handleSubmitSimplifiedBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		rpcsLog.Infof("Rejected block %s (Height %v) via submitblock: AddWitnessForSimplifiedBlock %v", block.Hash(), block.Height(), err)
		return nil, err
	}
	...
	_, err = s.cfg.SyncMgr.SubmitBlock(block, blockchain.BFNone)
	if err != nil {
		rpcsLog.Infof("Rejected block %s (Height %v) via submitblock: %v", block.Hash(), block.Height(), err)
		return fmt.Sprintf("rejected: %s", err.Error()), nil
	}

	rpcsLog.Infof("Accepted block %s (Height %v) via submitblock", block.Hash(), block.Height())
	return nil, nil
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="abec RPC"`)
...
func genCertPair(certFile, keyFile string) error {
	...
	if len(cfg.ExternalIPs) == 0 {
		rpcsLog.Infof("The ExternalIPs is empty, the generated certificate may be invalid for external rpc requests.")
	}
	extraHosts := cfg.ExternalIPs

	cert, key, err := abeutil.NewTLSCertPair(org, validUntil, extraHosts)
	if err != nil {
		return err
	}
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *abeutil.BlockAbe, flags blockchain.BehaviorFlags) (bool, error)
