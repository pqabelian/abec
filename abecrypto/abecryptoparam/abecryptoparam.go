...
func (cryptoScheme CryptoScheme) Serialize() []byte {
	res := make([]byte, 4)
	binary.LittleEndian.PutUint32(res, uint32(cryptoScheme))
	return res
}

// DeserializeCryptoScheme deserialize the input []byte with 4 bytes to a CryptoScheme.
// reviewed on 2023.12.12
func DeserializeCryptoScheme(serializedCryptoScheme []byte) (CryptoScheme, error) {
	if len(serializedCryptoScheme) != 4 {
		return CryptoSchemePQRingCT, fmt.Errorf("DeserializeCryptoScheme: invalid length")
	}
...
// CryptoSpendSecretKeyParse will be used to parse the input cryptoSpendSecretKey into (coin)SpendSecretKey,
// which was generated by CryptoAddressKeyGen or CryptoAddressKeyGenFromSeed.
// reviewed on 2023.12.12
func CryptoSpendSecretKeyParse(cryptoSpendSecretKey []byte) (coinSpendSecretKey []byte, err error) {
	...
	default:
		return nil, fmt.Errorf("CryptoSpendSecretKeyParse: non-supported cryptoScheme appears in serializedSpendSecretKey")
	}
}

// CryptoSerialNumberSecretKeyParse parses cryptoSerialNumberSecretKey into coinSerialNumberSecretKey.
// reviewed on 2023.12.12
func CryptoSerialNumberSecretKeyParse(cryptoSnSk []byte) (coinSerialNumberSecretKey []byte, err error) {
	cryptoScheme, err := ExtractCryptoSchemeFromCryptoSnsk(cryptoSnSk)
	if err != nil {
		return nil, err
	}
	...
	switch cryptoScheme {
	case CryptoSchemePQRingCT:
		return pqringctCryptoSerialNumberSecretKeyParse(PQRingCTPP, cryptoScheme, cryptoSnSk)

	default:
		return nil, fmt.Errorf("CryptoSerialNumberSecretKeyParse: non-supported cryptoScheme appears in cryptoSnSk")
	}
}

// CryptoValueSecretKeyParse parses cryptoValueSecretKey into coinValueSecretKey.
// reviewed on 2023.12.12
...
func ExtractCryptoSchemeFromCryptoSpsk(cryptoSpsk []byte) (cryptoScheme CryptoScheme, err error) {
	if len(cryptoSpsk) < 4 {
		return 0, fmt.Errorf("ExtractCryptoSchemeFromCryptoSpsk: incorrect length of cryptoSpsk: %d", len(cryptoSpsk))
	}
...
func ExtractCryptoSchemeFromCryptoSnsk(cryptoSnsk []byte) (cryptoScheme CryptoScheme, err error) {
	if len(cryptoSnsk) < 4 {
		return 0, fmt.Errorf("ExtractCryptoSchemeFromCryptoSnsk: incorrect length of cryptoSnsk: %d", len(cryptoSnsk))
	}

	return DeserializeCryptoScheme(cryptoSnsk[:4])
}

// ExtractCryptoSchemeFromCryptoVsk extracts cryptoScheme from cryptoVsk.
// reviewed on 2023.12.12
func ExtractCryptoSchemeFromCryptoVsk(cryptoVsk []byte) (cryptoScheme CryptoScheme, err error) {
	if len(cryptoVsk) < 4 {
		return 0, fmt.Errorf("ExtractCryptoSchemeFromCryptoVsk: incorrect length of cryptoVsk: %d", len(cryptoVsk))
	}
