...
type AbeTxInputDesc struct {
	...
	txoRing       *blockchain.TxoRing //	complete TxoRing information
...
func NewAbeTxInputDescWithFullRing(ringId *wire.RingId, txoRing *blockchain.TxoRing,
	sidx uint8, cryptoAddress []byte, cryptoSpsk []byte, cryptoSnsk []byte, cryptoVsk []byte, value uint64) *AbeTxInputDesc {
	return &AbeTxInputDesc{
...
func CheckCryptoAddress(cryptoAddress []byte) (valid bool, hints string) {
	...
	default:
		return false, "non-supported cryptoScheme appears in cryptoAddress"
	}
...
//	Note that these filled fields (except the Version) are independent from the underlying crypto scheme, and are specified by the issuer of a transaction.
