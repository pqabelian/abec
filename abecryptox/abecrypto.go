...
func TxoCoinReceive(abeTxo *wire.TxOutAbe, cryptoAddress []byte, cryptoVsk []byte) (bool, uint64, error) {
	...
	switch cryptoScheme {
	case abecryptoxparam.CryptoSchemePQRingCT:
		return abecrypto.TxoCoinReceive(abeTxo, cryptoAddress, cryptoVsk)
	case abecryptoxparam.CryptoSchemePQRingCTX:
		return pqringctTxoCoinReceive(abecryptoxparam.PQRingCTXPP, cryptoScheme, abeTxo, cryptoAddress, cryptoVsk)

	default:
		return false, 0, fmt.Errorf("TxoCoinReceive: unsupported crypto scheme")
	}
}
