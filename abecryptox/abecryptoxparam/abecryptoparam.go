...
func ExtractPrivacyLevelFromCryptoSpendSecretKey(cryptoScheme CryptoScheme, cryptoSpendSecretKey []byte) (privacyLevel PrivacyLevel, err error) {
	...
	case CryptoSchemePQRingCT:
		//	this is to achieve back-compatibility with PQRingCT
		_, _, err := abecryptoparam.CryptoAddressParse(cryptoAddress) // This call to make sure the input cryptoSpendSecretKey is well-formed.
		if err != nil {
			return 0, err
		}
		privacyLevel = PrivacyLevelRINGCTPre
...
func CryptoSpendSecretKeyParse([]byte cryptoSpSk)  (privacyLevel PrivacyLevel,
	coinSpendKey []byte,
	err error) {
	...
	default:
		return 0, nil, errors.New("ExtractPrivacyLevelFromCryptoAddress: unsupported crypto-scheme")
	}

	return privacyLevel, coinAddress, nil
