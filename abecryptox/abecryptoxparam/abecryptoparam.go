...
func GetCbTxWitnessSerializeSizeApprox(txVersion uint32, cryptoAddressListPayTo [][]byte) (int, error) {
	...
	case CryptoSchemePQRingCTX:
		...
		for i := 0; i < len(cryptoAddressListPayTo); i++ {
			...
			_, coinAddress, _, err := ParseCryptoAddress(cryptoAddressListPayTo[i])
			if err != nil {
				return 0, err
			}
...
func ParseCryptoAddress(cryptoAddress []byte) (privacyLevel PrivacyLevel,
	coinAddress []byte,
	valuePublicKey []byte,
	err error) {
	...
	case CryptoSchemePQRingCT:
		//	this is to achieve back-compatibility with PQRingCT
		coinAddress, valuePublicKey, err = abecryptoparam.ParseCryptoAddress(cryptoAddress)
		if err != nil {
			return 0, nil, nil, err
		}
		privacyLevel = PrivacyLevelRINGCTPre
