...
func pqringctxCryptoAddressParse(pp *pqringctxapi.PublicParameter, cryptoScheme CryptoScheme, cryptoAddress []byte) (
	...
	if cryptoSchemeInCryptoAddress != cryptoScheme {
		return 0, nil, nil, fmt.Errorf("pqringctxCryptoAddressParse: the input cryptoScheme does not match the cryptoScheme extracted from the input coinAddress")
	}
	...
	if privacyLevel == PrivacyLevelRINGCT {
		...
		valuePublicKey = make([]byte, coinValuePublicKeySize)
