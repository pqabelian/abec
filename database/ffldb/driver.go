...
func parseArgs(funcName string, args ...interface{}) (string, wire.AbelianNet, wire.NodeType, wire.TrustLevel, error) {
	if len(args) != 4 {
	...
	trustLevel, ok := args[3].(wire.TrustLevel)
	if !ok {
		return "", 0, 0, 0, fmt.Errorf("fourth argument to %s.%s is invalid -- "+
			"expected trust level", dbType, funcName)
	}

	return dbPath, network, nodeType, trustLevel, nil
}

// openDBDriver is the callback provided during driver registration that opens
// an existing database for use.
func openDBDriver(args ...interface{}) (database.DB, error) {
	dbPath, network, nodeType, trustLevel, err := parseArgs("Open", args...)
	if err != nil {
		return nil, err
	}

	return openDB(dbPath, network, nodeType, trustLevel, false)
}

// createDBDriver is the callback provided during driver registration that
// creates, initializes, and opens a database for use.
func createDBDriver(args ...interface{}) (database.DB, error) {
	dbPath, network, nodeType, trustLevel, err := parseArgs("Create", args...)
	if err != nil {
		return nil, err
	}

	return openDB(dbPath, network, nodeType, trustLevel, true)
}
