...
|29|[validateaddress](#validateaddress)|Y|Verifies the given cryptoAddress is valid.  NOTE: Since btcd does not have a wallet integrated, btcd will only return whether the cryptoAddress is valid or not.|
...
|Notes|NOTE: Since btcd does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in btcwallet for a txVersion which includes that information.|
