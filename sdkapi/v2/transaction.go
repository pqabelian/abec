...
type CryptoRootSeed struct {
	cryptoScheme                abecryptoxparam.CryptoScheme
	privacyLevel                abecryptoxkey.PrivacyLevel
	coinSpendKeyRootSeed        []byte // spend secret key, generated by CryptoAddressKeyGen
	...
	coinDetectorRootKey         []byte // detector key, generated by CryptoAddressKeyGen
}

type CryptoRandSeed struct {
	...
	cryptoDetectorKey []byte // detector key, generated by CryptoAddressKeyGen
}
...
func NewCryptoKey(cryptoAddress []byte, cryptoSpsk []byte, cryptoSnsk []byte, cryptoVsk []byte, cryptoDetectorKey []byte) *CryptoKey {
	return &CryptoKey{
		cryptoAddress:     cryptoAddress,
...
func readTxRequestOutputDesc(r io.Reader, txRequestOutputDesc *TxRequestOutputDesc) error {
	maxCoinAddressSize, err := abecryptoxkey.GetCoinAddressSize(abecryptoxparam.CryptoSchemePQRingCTX, abecryptoxkey.PrivacyLevelRINGCT)
	if err != nil {
		return err
	}
	coinValuePublicKeySize, err := abecryptoxkey.GetCoinValuePublicKeySize(abecryptoxparam.CryptoSchemePQRingCTX, abecryptoxkey.PrivacyLevelRINGCT)
	if err != nil {
		return err
	}
	macCryptoAddressSize := abecryptoxparam.CryptoSchemeSerializeSize() + 1 + maxCoinAddressSize + coinValuePublicKeySize
	//	cryptoAddress
	cryptoAddress, err := ReadVarBytes(r, uint32(macCryptoAddressSize), "TxRequestOutputDesc.CryptoAddress")
	if err != nil {
		return err
	}
...
func deserializeTransferTxRequestDesc(serializedTxRequestDesc []byte) (*TransferTxRequestDesc, error) {
	...
	// TxMemo []byte
	TxMemo, err := ReadVarBytes(r, abecryptoxparam.MaxAllowedTxMemoSize, "TransferTxRequestDesc.TxMemo")
	if err != nil {
		return nil, err
	}
...
func BuildTransferTxRequestDescFromBlocks(
	...
	for i := 0; i < outputNum; i++ {
		_, _, _, err = abecryptoxkey.CryptoAddressParse(txRequestOutputDescs[i].cryptoAddress)
		if err != nil {
			return nil, errors.New("BuildTransferTxRequestDesc: crypto address can not be parsed")
		}
	}
...
func CreateTransferTxByRootSeed(serializedTransferTxRequestDesc []byte, seeds *CryptoRootSeed) (serializedTxFull []byte, txId *TxId, err error) {
	...
	for i := 0; i < inputNum; i++ {
		...
		ok, value, err := abecryptox.TxoCoinReceiveByRootSeeds(txRequestInputDesc.txoRing.TxOuts[txRequestInputDesc.sidx], seeds.coinValueKeyRootSeed, seeds.coinDetectorRootKey)
		if err != nil {
			return nil, nil, err
		}
		if !ok {
		...
		abeTxInputDescs[i] = abecryptox.NewAbeTxInputDescByRootSeeds(
			...
			seeds.cryptoScheme,
			seeds.privacyLevel,
			seeds.coinSpendKeyRootSeed,
			seeds.coinSerialNumberKeyRootSeed,
			seeds.coinValueKeyRootSeed,
			seeds.coinDetectorRootKey,
...
func CreateTransferTx(serializedTransferTxRequestDesc []byte, cryptoKeys []*CryptoKey) (serializedTxFull []byte, txId *TxId, err error) {
	...
	if inputNum != len(cryptoKeys) {
		return nil, nil, fmt.Errorf("GenerateTransferTx: the number of input keys does not mathc the number of TransferTxRequestDesc's inputs")
	}
	...
	for i := 0; i < inputNum; i++ {
		...
		ok, value, err := abecryptox.TxoCoinReceiveByKeys(txRequestInputDesc.txoRing.TxOuts[txRequestInputDesc.sidx], cryptoKey.cryptoVsk, cryptoKey.cryptoDetectorKey)
		if err != nil {
			return nil, nil, err
		}
		if !ok {
...
func GenerateCoinSerialNumberByRootSeeds(
	outPoints []*OutPoint,
	serializedBlocksForRingGroup [][]byte,
	seeds *CryptoRootSeed) (serialNumbers [][]byte, err error) {
...
func GenerateCoinSerialNumberByKeys(
	outPoints []*OutPoint,
	serializedBlocksForRingGroup [][]byte,
