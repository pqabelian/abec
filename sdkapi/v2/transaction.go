...
type CryptoRootSeed struct {
	...
	coinSpendKeyRootSeed        []byte
	coinSerialNumberKeyRootSeed []byte
	coinValueKeyRootSeed        []byte
...
func NewCryptoKey(cryptoAddress []byte, cryptoSpsk []byte, cryptoSnsk []byte, cryptoVsk []byte, cryptoDetectorKey []byte) *CryptoKey {
	...
	if len(cryptoSnsk) == 0 {
		cryptoKey.cryptoSnsk = nil
	}
	if len(cryptoVsk) == 0 {
		cryptoKey.cryptoVsk = nil
	}
	if len(cryptoDetectorKey) == 0 {
		cryptoKey.cryptoDetectorKey = nil
	}
...
func CreateTransferTxByRootSeed(serializedTransferTxRequestDesc []byte, rootSeeds []*CryptoRootSeed) (serializedTxFull []byte, txId *TxId, err error) {
...
func CreateTransferTxByCryptoKeys(serializedTransferTxRequestDesc []byte, cryptoKeys []*CryptoKey) (serializedTxFull []byte, txId *TxId, err error) {
	txRequestDesc, err := deserializeTransferTxRequestDesc(serializedTransferTxRequestDesc)
	if err != nil {
		return nil, nil, err
	}
...
func GenerateCoinSerialNumberByRandSeeds(
	outPoints []*OutPoint,
	serializedBlocksForRingGroup [][]byte,
	randSeeds [][]byte) (serialNumbers [][]byte, err error) {
	...
	for i := 0; i < coinNum; i++ {
		txOut := txoRingMembers[i].txoRing.TxOuts[txoRingMembers[i].idx]
		serialNumbers[i], err = abecryptox.TxoCoinSerialNumberGenByRandSeed(txOut,
			*txoRingMembers[i].ringId,
			txoRingMembers[i].idx,
			randSeeds[i],
		)
