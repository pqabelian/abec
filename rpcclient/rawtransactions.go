...
func (c *Client) GetRawTransactionAsync(txHash *chainhash.Hash) FutureGetRawTransactionResult {
	...
	cmd := abejson.NewGetRawTransactionCmd(hash, abejson.Bool(false))
...
func (c *Client) GetRawTransactionVerboseAsync(txHash *chainhash.Hash) FutureGetRawTransactionVerboseResult {
	...
	cmd := abejson.NewGetRawTransactionCmd(hash, abejson.Bool(true))
	return c.sendCmd(cmd)
}
...
/*func (c *Client) CreateRawTransactionAsync(inputs []abejson.TransactionInput,
	...
	for addr, amount := range amounts {
		convertedAmts[addr.String()] = amount.ToABE()
	}
...
func (r FutureSignRawTransactionResult) Receive() (*wire.MsgTx, bool, error) {
	...
	var signRawTxResult abejson.SignRawTransactionResult
...
func (c *Client) SignRawTransactionAsync(tx *wire.MsgTx) FutureSignRawTransactionResult {
	...
	cmd := abejson.NewSignRawTransactionCmd(txHex, nil, nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SignRawTransaction2Async(tx *wire.MsgTx, inputs []abejson.RawTxInput) FutureSignRawTransactionResult {
	txHex := ""
	if tx != nil {
		// Serialize the transaction and convert to hex string.
		buf := bytes.NewBuffer(make([]byte, 0, tx.SerializeSize()))
	...
	cmd := abejson.NewSignRawTransactionCmd(txHex, &inputs, nil, nil)
	return c.sendCmd(cmd)
}
...
// transactions.
func (c *Client) SignRawTransaction2(tx *wire.MsgTx, inputs []abejson.RawTxInput) (*wire.MsgTx, bool, error) {
...
func (c *Client) SignRawTransaction3Async(tx *wire.MsgTx,
	...
	cmd := abejson.NewSignRawTransactionCmd(txHex, &inputs, &privKeysWIF,
		nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SignRawTransaction4Async(tx *wire.MsgTx,
	inputs []abejson.RawTxInput, privKeysWIF []string,
	...
	cmd := abejson.NewSignRawTransactionCmd(txHex, &inputs, &privKeysWIF,
		abejson.String(string(hashType)))
	return c.sendCmd(cmd)
}
...
// Receive waits for the response promised by the future and returns the
// found raw transactions.
func (r FutureSearchRawTransactionsVerboseResult) Receive() ([]*abejson.SearchRawTransactionsResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var result []*abejson.SearchRawTransactionsResult
...
/*func (c *Client) SearchRawTransactionsVerboseAsync(address abeutil.Address, skip,
	...
	addr := address.EncodeAddress()
	verbose := abejson.Int(1)
	var prevOut *int
	if includePrevOut {
		prevOut = abejson.Int(1)
	}
	cmd := abejson.NewSearchRawTransactionsCmd(addr, verbose, &skip, &count,
...
func (r FutureDecodeScriptResult) Receive() (*abejson.DecodeScriptResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var decodeScriptResult abejson.DecodeScriptResult
...
func (c *Client) DecodeScriptAsync(serializedScript []byte) FutureDecodeScriptResult {
	...
	cmd := abejson.NewDecodeScriptCmd(scriptHex)
	return c.sendCmd(cmd)
}

// DecodeScript returns information about a script given its serialized bytes.
func (c *Client) DecodeScript(serializedScript []byte) (*abejson.DecodeScriptResult, error) {
