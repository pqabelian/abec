...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *abejson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *abejson.NotifyNewTransactionsCmd:
...
type rawResponse struct {
	...
	Error  *abejson.RPCError `json:"error"`
}
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]abejson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := abejson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := abejson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
