...
type notificationState struct {
	...
	notifySpent    map[abejson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[abejson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[abejson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount abeutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance abeutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	*/
	...
	//
	//// OnRedeemingTx
	//case abejson.RedeemingTxNtfnMethod:
	//	// Ignore the notification if the client is not interested in
	//	// it.
	...
	//case abejson.RelevantTxAcceptedNtfnMethod:
	//	// Ignore the notification if the client is not interested in
	//	// it.
	...
	// OnRescanFinished
	case abejson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case abejson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case abejson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case abejson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
/*func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*abeutil.Tx, error) {
	...
	transactions := make([]*abeutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = abeutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
/*func parseChainTxNtfnParams(params []json.RawMessage) (*abeutil.Tx,
	*abejson.BlockDetails, error) {
	...
	// JSON object.
	var block *abejson.BlockDetails
	if len(params) > 1 {
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*abejson.TxRawResult,
	error) {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance abeutil.Amount, confirmed bool, err error) {
...
func (c *Client) notifySpentInternal(outpoints []abejson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := abejson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the btcjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) abejson.OutPoint {
	return abejson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]abejson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := abejson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := abejson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := abejson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
/*func (c *Client) LoadTxFilterAsync(reload bool, addresses []abeutil.Address,
	...
	outPointObjects := make([]abejson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = abejson.OutPoint{
