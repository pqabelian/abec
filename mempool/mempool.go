...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *abeutil.Tx, height int32, fee int64) *TxDesc {
...
func (mp *TxPool) maybeAcceptTransaction(tx *abeutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	...
	var conflicts map[chainhash.Hash]*abeutil.Tx
	if isReplacement {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *abeutil.Tx) []*TxDesc {
