...
func (b *BlockChain) processOrphansAbe(hash *chainhash.Hash, flags BehaviorFlags) error {
	...
	for len(processHashes) > 0 {
		...
		// reevaluate the slice on each iteration nor does it adjust the
		// index for the modified slice.
		for i := 0; i < len(b.prevOrphansAbe[*processHash]); i++ {
			orphan := b.prevOrphansAbe[*processHash][i]
			if orphan == nil {
				log.Warnf("Found a nil entry at index %d in the "+
					"orphan dependency list for block %v", i,
					processHash)
...
// This function is safe for concurrent access.
//	todo(ABE): Remove
//func (b *BlockChain) ProcessBlockBTCD(block *abeutil.Block, flags BehaviorFlags) (bool, bool, error) {
//	b.chainLock.Lock()
