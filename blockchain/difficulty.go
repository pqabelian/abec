...
// say that, it supports only this smooth factor array, i.e.,
// the difficulty of coming slot will be affected/computed from the immediate 20 slots (of blocks),
// where the number of blocks in each slot is decided by TargetTimespanDSA and TargetTimePerBlockDSA.
...
//
// This compact form is only used in abelian to encode unsigned 256-bit numbers
// which represent difficulty targets, thus there really is not a need for a sign bit,
...
// values larger than (2^23 - 1) only encode the most significant digits of the number.
// See CompactToBig for details.
func BigToCompact(n *big.Int) uint32 {
	...
	// Since the base for the exponent is 256, the exponent can be treated as the number of bytes.
	// So, shift the number right or left accordingly.  This is equivalent to:
...
// Abelian increases the difficulty for generating a block by decreasing the value which the generated hash must be less than.
...
// Since a lower target value equates to higher actual difficulty,
// the work value which will be accumulated must be the inverse of the difficulty.
// Also, in order to avoid potential division by zero and small floating point numbers,
// the result adds 1 to the denominator and multiplies the numerator by 2^256.
func CalcWork(bits uint32) *big.Int {
	// Return a work value of zero if the passed difficulty bits represent a negative number.
	// Note this should not happen in practice with valid blocks, but an invalid block could trigger it.
...
func (b *BlockChain) calcNextRequiredDifficulty(lastNode *blockNode, newBlockTime time.Time) (uint32, error) {
	...
	if lastNode.height+1 >= b.chainParams.BlockHeightDSA {
		...
		return b.calcNextRequiredDifficultyDSA(lastNode, newBlockTime)
	}

	return b.calcNextRequiredDifficultyInit(lastNode, newBlockTime)
}

// calcNextRequiredDifficultyDSA calculates the required difficulty for the block
// after the passed previous block node based on the difficulty retarget rules.
//
// This function is a subroutine for calcNextRequiredDifficulty,
// for the case of lastNode.height+1 >= b.chainParams.BlockHeightDSA,
// where DSA algorithms are applied.
func (b *BlockChain) calcNextRequiredDifficultyDSA(lastNode *blockNode, newBlockTime time.Time) (uint32, error) {
	// Genesis block.
	if lastNode == nil {
		return b.chainParams.PowLimitBits, nil
	}
	...
	if lastNode.height+1 < b.chainParams.BlockHeightDSA {
		errStr := fmt.Sprintf("wrong call on calcNextRequiredDifficultyDSA: lastNode.height+1 (%d) < b.chainParams.BlockHeightDSA (%d)",
...
// calcNextRequiredDifficultyInit calculates the required difficulty for the block
// after the passed previous block node based on the difficulty retarget rules.
//
// This function is a subroutine for calcNextRequiredDifficulty,
// for the case of lastNode.height+1 < b.chainParams.BlockHeightDSA,
// i.e., the initial difficulty-adjustment-mechanism.
func (b *BlockChain) calcNextRequiredDifficultyInit(lastNode *blockNode, newBlockTime time.Time) (uint32, error) {
	// Genesis block.
	if lastNode == nil {
		return b.chainParams.PowLimitBits, nil
	}
	...
	if lastNode.height+1 >= b.chainParams.BlockHeightDSA {
		errStr := fmt.Sprintf("wrong call on calcNextRequiredDifficultyInit: lastNode.height+1 (%d) >= b.chainParams.BlockHeightDSA (%d)",
			lastNode.height+1, b.chainParams.BlockHeightDSA)
		return 0, AssertError(errStr)
	}
	...
	// currentDifficulty * (adjustedTimespan / targetTimespan)
	// The result uses integer division which means it will be slightly rounded down.
	// Bitcoind also uses integer division to calculate this result.
	...
	log.Infof("Actual timespan %v, adjusted timespan %v, target timespan %v",
		time.Duration(actualTimespan)*time.Second, time.Duration(adjustedTimespan)*time.Second, b.chainParams.TargetTimespan)

	return newTargetBits, nil
}

// calcTargetFromExpectedWorkPerBlock computes the target from the expected work per block,
// i.e.,  the number of nonce tried to find a valid block (with hash-number smaller than the result target).
func calcTargetFromExpectedWorkPerBlock(expectedWorkPerBlock *big.Int) *big.Int {
	denominator := big.NewInt(1)
	if expectedWorkPerBlock != nil && expectedWorkPerBlock.Sign() > 0 {
		denominator = expectedWorkPerBlock
	}
...
// can have given starting difficulty bits and a duration.
...
func (b *BlockChain) calcEasiestDifficulty(bits uint32, duration time.Duration) uint32 {
	...
	// than twice the desired amount of time needed to generate a block has elapsed.
	if b.chainParams.ReduceMinDifficulty {
		reductionTime := int64(b.chainParams.MinDiffReductionTime /
			time.Second)
	...
	// Since easier difficulty equates to higher numbers,
	// the easiest difficulty for a given duration is the largest value possible given
...
// calcEasiestDifficultyDSA calculates the easiest possible difficulty that a block
// can have given starting difficulty bits and a duration.
// It is mainly used to verify that claimed proof of work by a block is sane as compared to a known good checkpoint.
func (b *BlockChain) calcEasiestDifficultyDSA(bits uint32, duration time.Duration) uint32 {
	// Convert types used in the calculations below.
	durationVal := int64(duration / time.Second)
	adjustmentFactor := big.NewInt(b.chainParams.RetargetAdjustmentFactor)
	...
	for durationVal > 0 && newTarget.Cmp(b.chainParams.PowLimit) < 0 {
		newTarget.Mul(newTarget, adjustmentFactor)
		durationVal -= b.maxRetargetTimespanDSA
	}
