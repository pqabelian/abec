...
func storeFilter(dbTx database.Tx, block *abeutil.Block, f *gcs.Filter,
...
//	Abe to do
func storeFilterAbe(dbTx database.Tx, block *abeutil.BlockAbe, f *gcs.Filter,
	filterType wire.FilterType) error {
	if uint8(filterType) > maxFilterType {
		return errors.New("unsupported filter type")
	}
...
func (idx *CfIndex) ConnectBlock(dbTx database.Tx, block *abeutil.Block,
...
func (idx *CfIndex) ConnectBlockAbe(dbTx database.Tx, block *abeutil.BlockAbe,
	...
	return storeFilterAbe(dbTx, block, f, wire.GCSFilterRegular)
}
