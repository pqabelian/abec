...
func calcInputValueAgeAbe(tx *wire.MsgTxAbe, utxoRingView *blockchain.UtxoRingViewpoint, nextBlockHeight int32) float64 {
	var totalInputAge float64
	for _, txIn := range tx.TxIns {
		...
		if entry != nil && !entry.IsSpent(txIn.SerialNumber) {
			...
			/*			inputValue := entry.Amount()
						totalInputAge += float64(inputValue * int64(inputAge))*/
			totalInputAge += float64(int64(inputAge))
		}
	}
