...
// encodeAddress returns a human-readable payment address given a pripemd160 hash
...
//	todo(ABE): add netID support, because of different network we will use.
//	DecodeMasterAddressAbe decodes a human-readable string (say, mining address or pay-to-address) to a MasterAddress.
//	This function may be implemented according to different rules.
//	At this moment, the addrStr is generated by hex.EncodeToString(serilaized MasterAddress, hash for checksum).
func DecodeMasterAddressAbe(addrstr string) (MasterAddress, error) {
	if len(addrstr) == 0 {
		//	If the caller wants to use nil master address, do not call this function.
	...
	totalSize := len(addrBytes)
	if totalSize <= 2+chainhash.HashSize { //TODO(osy): 2 + chainhash.HashSize consider nil of address?
		return nil, errors.New("decoded address is of unknown format: bytesize <= 2 + chainhash.HashSize")
	}

	cryptoScheme := abecrypto.CryptoScheme(binary.BigEndian.Uint16(addrBytes[:2]))
...
//	DecodeDerivedAddressAbe decodes a human-readable string (say, in a transaction dispalyed on webpage) to a DerivedAddress.
//	This function may be implemented according to different rules.
//	At this moment, the addrStr is generated by hex.EncodeToString(serilaized DerivedAddress).
...
//	todo(ABE):
type AddressScriptFormat uint8

const (
	AddressScriptDerivedAddress AddressScriptFormat = iota
	AddressScriptDerivedAddressHash
)
